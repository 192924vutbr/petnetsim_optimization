...
def run():
    subnet_ctx = pns.new_context()
    with open('sample_012_netclone_cython.json') as f:
        sn_places, sn_transitions, sn_arcs, sn_graphics = petnetsim.json_io.load(f, subnet_ctx)
        subnet = pns.PetriNet(sn_places, sn_transitions, sn_arcs, subnet_ctx)

    places = [pns.Place('Waves', 10)]
    transitions = [pns.Transition('make_wave')]
    ...
    petri_net = pns.PetriNet(places, transitions, arcs)
